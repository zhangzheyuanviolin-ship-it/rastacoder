...
python3 scripts/prepare_curl_cffi_native_companion_v16.py | tee /tmp/v17-native-prep.log
grep -F "required companion: $PRIVATE_CURL_LIB" /tmp/v17-native-prep.log
...
gradle -p android :app:generateDebugPythonRequirements --no-daemon 2>&1 | tee /tmp/v17-release-python-resolution.log
verify_dep() {
  local pattern="$1"
  if grep -Eiq "$pattern" /tmp/v17-release-python-resolution.log; then return 0; fi
...
cp android/app/build.gradle /tmp/v17-build.gradle-before-signing
python3 - <<'PY'
from pathlib import Path
...
# Exactly one formal APK build in V17.
...
cp /tmp/v17-build.gradle-before-signing android/app/build.gradle

echo "Final V17 APK SHA-256: $OUT_SHA"
echo "Final V17 APK size: $OUT_SIZE"
echo 'V17 release phase 2A passed: one APK built and package/signature/ABI/MLC/curl-cffi native payload verified.'
